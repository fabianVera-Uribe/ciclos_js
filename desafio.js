...
function saludo(idioma) {
  ...
  const language = idioma.toLowerCase();
...
function cuentoElementos(arreglo) {
  ...
  return contador;
  // let mayoresADieciOcho = arreglo.filter((elemento) => elemento > 18);
  // return mayoresADieciOcho.length;
}
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  const index1 = n.toString().charAt(0);
  if (index1 === "9") {
    return true;
  } else {
    return false;
...
function mayorACien(array) {
  ...
  // Tu código:
  const mayoresACien = array.filter((num) => num > 100);
  return mayoresACien;
}
